...
WINDOW_SIZE = (840,600) # Window size (X,Y) in pixels
...
myfont = pygame.font.SysFont("retro",12)


pygame.display.set_caption(' [CyberKnight v1 ]') # Set the window title.
...
grass0_img = pygame.image.load('tiles/background/grass_0.png').convert()#these grass to be a router and the clouds to be a satelite
...
goomba_img = pygame.image.load('tiles/enemy/goomba/goomba_run/goomba_run_x.png').convert()
...
questions = [['What is cybersecurity?', 'yes'], ['1)cybersecurity is protection of hardware ,software,and data from attackers.','yes'] ,[ "2) information security ,network security ,operational security,are some of the cybersecurity elements? ", 'yes' ],
              [ ' business continuity is not a  benefit of cubersecurity ?', ' no '], ['cia  is for confidentiality , integrity and availability ','yes'], [' is it true a firewall is only hardware  and protects against viruses only?', ' no'] ,
              ['brute force is a way of cracking passwords?', 'yes']]
...
#         exit()
    ...
    choose = dm.dumbmenu(screen, [
        'GAME OVER ',
        'Quit Game'], 250, 200, None, 32, 1.4, black, red)
    if choose == 0:
...
class Button:
        def __init__(self, text, width, height, pos, elevation):
            ...
            # text
            self.text_surf = myfont.render(text, True, '#FFFFFF')
...
choose = dm.dumbmenu(screen, [
                        'Start Game',
                        'Quit Game', ' Pause  Game'], 250,200,None,32,1.4,green,red)
...
renderedQuestion = False

while not IsGameOver or True: # Main game loop

    if IsGameOver == True:
        labely = myfont.render("game over ", 500, (0, 0, 0))
        screen.blit(labely, (320,300))
        print("1")
    ...
    if renderQuestion:
        if renderedQuestion:
            ...
            selectedQuestion = random.randrange(0, len(questions))
            question = questions[selectedQuestion][0]
            answer = questions[selectedQuestion][1]

        choose = dm.dumbmenu(screen,[
            question,
            'Yes',
            'No'], 200, 200, None, 32, 1.4, green, red
        )
        if answer == "Yes":
            pos = 1
        else:
    ...
    if player_rect.y > 216:
        ...
        while IsGameOver == True:
            labely = myfont.render("game over ", 250, (0, 0, 0))
            screen.blit(labely, (10, 150))
            choose = dm.dumbmenu(screen, [
                'Start Game',
                'Quit Game'], 250, 200, None, 32, 1.4, green, red)
            if choose == 1:
    ...
    scorelabel = myfont.render('CYBER SCORE:' + str(score), 1, (255,255,255))
    display.blit(scorelabel, (200,10))
    ...
    for brick in bricks:
        ...
        if brick.isbumping:
           ...
           #
           # kalabel = myfont.render('kalabel:' + str(int(clock.get_fps())), 500, (0, 0, 0))
           # screen.blit(kalabel, (150, 150))
           #
           # #    screen.fill((blue))
           # button1 = Button('Click me', 200, 40, (100, 250), 5)
           # button1.draw()
